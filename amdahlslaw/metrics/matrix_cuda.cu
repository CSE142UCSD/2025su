...
int main(int argc, char const *argv[])
{
  ...
  if(c == NULL)
   fprintf(stderr,"allocating array c failed\n");
    ...
    printf("Time elapsed on matrix multiplication of %ld %dx%d . %dx%d on GPU: %f ms.\n\n", sizeof(D_TYPE), ARRAY_SIZE, ARRAY_SIZE, ARRAY_SIZE, ARRAY_SIZE, gpu_elapsed_time_ms);
  const float flops = (float)(2.0f * ARRAY_SIZE * ARRAY_SIZE * ARRAY_SIZE/(gpu_elapsed_time_ms/1e3));
  printf("\nThroughput: %0.2f GFLOPS\n\n", flops * 1e-9);
  fprintf(stderr, "%ld, %dx%dx%d, %f, %0.2f\n",sizeof(D_TYPE), ARRAY_SIZE, ARRAY_SIZE, ARRAY_SIZE, gpu_elapsed_time_ms, flops * 1e-9);
/*
